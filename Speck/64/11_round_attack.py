...
def rotate_nbs(pos=32, offset=3, word_size=32):
    if pos < word_size:
        return (pos + offset) % word_size
    else:
        return word_size + ((pos + offset) % word_size)


def rotate_differentials(diff=(0x0, 0x0), offset=None):
    new_diff = (sp.rol(diff[0], offset), sp.rol(diff[1], offset))
    return new_diff
...
if __name__ == '__main__':
    ...
    # for 11-round speck64/
    base_diff = (0x81248000, 0x00802084)
    base_nbs = [[7, 47], [13, 53], [17], [18], [19], [56], [57], [58], [59], [60]]
    o1, o2, o3 = 10, 15, 1
    diff_1 = rotate_differentials(diff=base_diff, offset=o1)
    diff_2 = rotate_differentials(diff=base_diff, offset=o2)
    diff_3 = rotate_differentials(diff=base_diff, offset=o3)
    NB_1 = [[rotate_nbs(pos=i, offset=o1, word_size=32) for i in nb] for nb in base_nbs]
    NB_2 = [[rotate_nbs(pos=i, offset=o2, word_size=32) for i in nb] for nb in base_nbs]
    NB_3 = [[rotate_nbs(pos=i, offset=o3, word_size=32) for i in nb] for nb in base_nbs]
    print(NB_1)
    print(NB_2)
    print(NB_3)
    print(hex(diff_1[0]), hex(diff_1[1]))
    print(hex(diff_2[0]), hex(diff_2[1]))
    print(hex(diff_3[0]), hex(diff_3[1]))
    attack_with_dual_NDs(t=100, nr=11, diffs=(diff_1, diff_2, diff_3), NBs=(NB_1, NB_2, NB_3), nds=(nd1, nd2, nd3),
